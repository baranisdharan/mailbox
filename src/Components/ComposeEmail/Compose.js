  ...
  const SendHandler = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(
        `https://ecommerce-app-26adc-default-rtdb.firebaseio.com/${toFormattedEmail}inbox.json`,
        mailData
      );
    ...
    try {
      const response = await axios.post(
        `https://ecommerce-app-26adc-default-rtdb.firebaseio.com/${newMAil}sentbox.json`,
        {
          to: toFormattedEmail,
          subject: enteredSubjectref,
          body: bodyText,
        ...
        <div className={classes.wrapper}>
          <form>
            ...
            <div className={classes.registration}>
              {" "}
              <Editor onEditorStateChange={EditorStateChangeHandler} />
            </div>
