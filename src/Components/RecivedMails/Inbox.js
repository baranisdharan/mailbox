...
const ReceiveEmails = () => {
  ...
  async function GetData() {
    try {
      let res = await axios.get(
        `https://ecommerce-app-26adc-default-rtdb.firebaseio.com/${FormatedEmail}inbox.json`
      );
  ...
  async function DeleteHandler(id) {
    ...
    const res = await axios.delete(
      `https://ecommerce-app-26adc-default-rtdb.firebaseio.com/${FormatedEmail}inbox/${id}.json`
    );
    let data = await res;
    console.log(data);
                  ...
                  return (
                    <li className={classes.input}>
                      ...
                      <Link to={`/Inbox/${email.id}`}>
                        {" "}
                        <span style={{ marginRight: "1em" }}>{email.from}</span>
